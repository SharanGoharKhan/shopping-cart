...
const SELECT_ADDRESS = gql`${selectAddress}`

function Card(props) {
    const navigation = useNavigation()
    const isDefault = props.default
    const [mutate, { loading: loadingMutation }] = useMutation(DELETE_ADDRESS)
    const [mutateSelected, { loading: loadingSelected }] = useMutation(SELECT_ADDRESS, { onError })
    ...
    function renderUnselectedButton() {
        return (
            <MainBtn
                ...
                onPress={() => { onSelectAddress(props.item) }}
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.address}>
                ...
                <TextDefault textColor={colors.fontMainColor}>
                    {props.item.apartment}{', '}{props.item.building}
                </TextDefault>
